...
### sudo
Чтобы что-то сделать в баше с правами администратора нужно использовать комкнду `sudo`.
...
### Симлинки
Символьные ссылки (симлинки, symbolyc links).
Например, можно назначить шорткат на запуск текстового редактора: http://olivierlacan.com/posts/launch-sublime-text-3-from-the-command-line/

# chown
Изменить владельца файла или папки.

### TODO
* Пакетные менеджеры на Маке: установить `brew` и добавить команду `tree` для древовидного вывода.
* chmod, chown
* sudo
* Симлинки
* Работа с БД: импорт, изменение урлов в `core_config_data`.
* Работа с сервером: запустить, остановить, перезапустить, добавить хост. 
* open . и open some/file.html, запуск sublime из баша
