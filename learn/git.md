...
`staging area` — здесь хранятся изменения, которые попадут в репозиторий при коммите.
`untracked` — файл существует, но не добавлен ни в репозиторий, ни в staging area.
`SHA` — хэш (набор букв и цифр) коммита. Генерируется при коммите изменений. По SHA можно переходить к определенному коммиту в истории изменений.
`HEAD` — ссылка на последний коммит в текущей ветке. По умолчанию ссылается на самый последний.
`HEAD^` — предпоследний коммит. Есть еще `HEAD^^` — пред-предпоследний коммит.

## Staging Area
`git add fileName` — добавить в SA.
`git add --all` — добавить все изменения и новые файлы в SA. Так же работает `git add .`.
`git reset myFile.html` — убрать добавленный в staging area файл из staging area. Изменеия в файле не потеряются, просто они не будут учтены при коммите.
`git add *.txt` — добавить все файлы `.txt` из _текущего каталога_.
`git add "*.txt"` — добавить все файлы `.txt` _из всего проекта_.

## Commit
Сообщения о коммитах добны быть в настоящем времени: Добавить файл, Изменить мыло. Коммит должен ответить на вопрос, что он делает, а не что он сделал. Подробнее см. в [Вики](http://w.oggettoweb.com/index.php?title=%D0%98%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5_git).
...
`git checkout -- filename.html` — убрать изменения в `filename.html` до последнего коммита. `git checkout -- .` — откатить все файлы до последнего коммита.
`git reset --soft HEAD^` — отменить коммит и оставить изменения в Staging Area. Если сделали коммит, но потом вспомнили, что там не все. `HEAD^` означает коммит перед текущим. `--soft` говорит, что нужно оставить изменения в Staging Area.
`git commit --amend` — добавить изменения из Staging Area к предыдущему коммиту. Если вдруг забыли добавить файл в сделаный коммит. Можно даже перезаписать коммит-сообщение: `git commit --amend -m "New commit message"`.
`git reset --hard HEAD^` — удалить последний коммит и все изменения в нем. Нигде ничего не сохранится.
...
recursive strategy — слияние веток, когда изменения есть в обоих ветках. Там коммиты и тут коммиты. Делается merge commit и изменения сливаются.
...
# Тэги
tag — референс на какой-то коммит. Чтобы удобно было на него скакнуть. Часто теги используются для нумерации релизов.

Разобраться подробнее, как принято работать с тегами.

`git tag` — показать список тэгов.
`git checkout tagName` — переключиться на тэг (определенный именованный коммит).
...
Когда мержатся 2 ветки и в обоих есть коммиты, то получается merge commit. Это точка на графе, которая, по сути, не несет никакой полезной нагрузки. Merge commit не про код, а чисто техническая деталь.

Чтобы избежать мерж-коммитов можно делать ребейз.
...
2. Применяет все коммиты из `origin/master`.
3. Применяет все локальные коммиты из временной зоны (которые были в локальном мастере) по одному.
...
2. Запускаем `git rebase master`. Эта команда добавит все коммиты из `master` в `my-branch`.
3. Переходим в `master` и мержим в него `my-branch`: `git merge my-branch`. Получается fast-forward.

# Удаление файлов
* `git rm "*.txt"` — удалить файлы (папки, файл) из репозитория и добавить удаление в коммит.
...
`git diff <SHA>` — посмотреть различия того, что есть и того, что в коммите `<SHA>`. `git diff HEAD` — ссылается на текущий бранч, как и `git diff`. Работает не только с коммитами.
...
# Внимание!
* Не стоит изменять историю коммитов, если они были запушены в удаленный репозиторий. Потом будут большие проблемы при получении изменений.
