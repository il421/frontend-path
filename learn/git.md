...
`HEAD` — ссылка на последний комит в текущей ветке. По умолчанию ссылается на самый последний. `HEAD^` — предпоследний комит. Есть еще `HEAD^^` — пред-предпоследний комит.
...
`git reset --hard HEAD^` — удалить _последний комит_ и все изменения в нем. Нигде ничего не сохранится.
