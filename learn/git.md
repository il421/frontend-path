...
`git checkout -- filename.html` — убрать изменения в `filename.html` до последнего комита. `git checkout -- .` — откатить все файлы до последнего комита. Здесь `--` говорит, что не надо воспринимать `filename.html` как имя ветки. Отделяет опцию от параметра. Аналогично `git checkout HEAD filename.html`.
