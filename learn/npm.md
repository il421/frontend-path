...
Для того, чтобы NPM не искал обновления установленных пакетов (при релизе может быть не безопасно), применяют утилиту [`npm shrinkwrap`](https://docs.npmjs.com/cli/shrinkwrap).

По умолчанию `npm shrinkwrap` не залочит `devDependencies`. Чтобы их залочить, нужно добавить флаг `--dev`:

```bash
npm shrinkwrap --dev
```

* [Документация по shrinkwrap](https://docs.npmjs.com/cli/shrinkwrap)
* [Статья про организацию зависимостей](https://docs.npmjs.com/cli/shrinkwrap)
